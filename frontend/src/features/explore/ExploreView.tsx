  ...
  return (
    <div
      ...
      onPointerDownCapture={(event) => {
        ...
        if (ctl.siteFilterRef.current?.contains(event.target as Node)) return;
      ...
      <div className="card bg-transparent text-foreground border-0 h-full content-shell-card">
        <div className="card-body">
              ...
              {ctl.sort === 'popular' ? (
                <>
                  <select
                    ...
                    onChange={(event) =>
                      ctl.setPopularWindow(event.target.value as ExploreWindow)
                    }
            ...
            <div className="gallery-control-group flex items-center gap-2">
              ...
              <div className="dropdown" ref={ctl.siteFilterRef}>
                <button
                  ...
                  onClick={() => ctl.setIsSiteFilterOpen(!ctl.isSiteFilterOpen)}
                >
          ...
          {ctl.siteErrors.map((siteError) => (
            <div key={siteError.siteId} className="text-sm mb-2" role="status">
                    ...
                    {column.map((post) => {
                      ...
                      return (
                        <ExploreCard
                          ...
                          canFavorite={
                            ctl.siteById.get(post.siteId)?.canFavorite ?? false
                          }
                          favorited={ctl.isFavorited(post)}
                          voted={ctl.voteOf(post)}
                          busy={ctl.pendingActionKey === key}
