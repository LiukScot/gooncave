...
test('voting locks the buttons, and the Extra toggles hide Games + Rated', async ({
  ...
  // renders the defaults until the server answers, so reading a checkbox
  // right after a load can report the default rather than the stored value.
  type ExtraSettings = {
  ...
  // assertion failing mid-way must not leave them changed for everything
  // after. The vote system is off by default, so voting needs it on first.
  try {
    await writeSettings({ voteSystemEnabled: true });
